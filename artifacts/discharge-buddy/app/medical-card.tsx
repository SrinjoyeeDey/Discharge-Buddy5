...
const WHITE = "#ffffff";
const ACCENT = "#00B894";
const BASE_URL = "https://discharge-buddy.com";
...
export default function MedicalCardScreen() {
    ...
    const getPublicUrl = (card: MedicalCard) => {
        return `${BASE_URL}/public/medical-card/${card.id}`;
    };
    ...
    const handleShareQRLink = async () => {
        ...
        try {
            const publicLink = getPublicUrl(medicalCard);
            if (Platform.OS === "web") {
            ...
            >
                ...
                <View style={styles.qrSection}>
                    ...
                    <View style={styles.qrContainer}>
                        {/* ✅ QR ALWAYS GENERATES USING PUBLIC URL */}
                        <QRCode
                            ref={setQrRef}
                            value={getPublicUrl(medicalCard)}
                            size={200}
                            color={TEAL_DARK}
                    ...
                    <Text style={styles.qrHint}>
                        Scan this QR code with Google Lens or any camera to instantly access medical information.
                    </Text>
                    <Text style={styles.qrUrl}>
                        🔗 {getPublicUrl(medicalCard)}
                    </Text>
                ...
                <View style={styles.card}>
                    ...
                    {editing ? (
                        <>
                            <EditField label="Blood Type" value={bloodType} onChangeText={setBloodType} placeholder="O+" />
                            <EditField label="Allergies" value={allergies} onChangeText={setAllergies} placeholder="e.g., Penicillin, Nuts" multiline />
                            <EditField label="Conditions / Diseases" value={diseases} onChangeText={setDiseases} placeholder="e.g., Diabetes, Hypertension" multiline />
                        </>
                    ) : (
                        <>
                ...
                <View style={styles.card}>
                    ...
                    {editing ? (
                        <>
                            <EditField label="Contact Name" value={ecName} onChangeText={setEcName} placeholder="Name" />
                            <EditField label="Contact Phone" value={ecPhone} onChangeText={setEcPhone} placeholder="+1 (555) 000-0000" keyboardType="phone-pad" />
                        </>
                    ) : (
                        <>
                ...
                {!editing && (
                    <View style={styles.card}>
                        ...
                        <InfoRow label="Times Accessed" value={medicalCard.accessCount?.toString() || "0"} />
                        <InfoRow label="Last Accessed" value={medicalCard.accessedAt ? new Date(medicalCard.accessedAt).toLocaleDateString() : "Never"} />
                    </View>
                )}
                ...
                {!editing && (
                    <View style={styles.dangerZone}>
                        <TouchableOpacity style={styles.dangerBtn} onPress={handleDisableCard} disabled={loading}>
                ...
                <View style={styles.infoNote}>
                    ...
                    <Text style={styles.infoNoteText}>
                        Your medical card is encrypted and secure. Scan the QR code with Google Lens or any camera app to access details instantly.
                    </Text>
                </View>
            </ScrollView>
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <Text style={styles.modalSubtitle}>
                            Show this to emergency responders. Scan with Google Lens for instant access.
                        </Text>
                    </View>
                </View>
            </Modal>
...
function InfoRow({ label, value, highlight, danger }: { label: string; value: string; highlight?: boolean; danger?: boolean; }) {
    return (
        <View style={styles.infoRow}>
            ...
            <Text style={[styles.infoValue, highlight && styles.infoValueHighlight, danger && styles.infoValueDanger]}>{value}</Text>
        </View>
    );
}

function EditField({ label, value, onChangeText, placeholder, keyboardType = "default", multiline = false }: { label: string; value: string; onChangeText: (text: string) => void; placeholder?: string; keyboardType?: string; multiline?: boolean; }) {
    return (
...
const styles = StyleSheet.create({
    ...
    header: { paddingHorizontal: 20, paddingBottom: 24, borderBottomLeftRadius: 40, borderBottomRightRadius: 40, overflow: "hidden" },
    ...
    emptyBackBtn: { position: "absolute", top: 50, left: 20, width: 38, height: 38, borderRadius: 19, backgroundColor: `${TEAL}15`, alignItems: "center", justifyContent: "center" },
    headerCenter: { alignItems: "center", flexDirection: "row", justifyContent: "center", gap: 8 },
    headerEmoji: { fontSize: 22 },
    headerTitle: { fontSize: 22, fontFamily: "Inter_700Bold", color: WHITE },
    editBtn: { width: 38, height: 38, borderRadius: 19, backgroundColor: WHITE, alignItems: "center", justifyContent: "center" },
    headerSub: { fontSize: 12, fontFamily: "Inter_400Regular", color: "rgba(255,255,255,0.8)", textAlign: "center", marginTop: 8 },
    content: { padding: 16, paddingBottom: 40, gap: 16 },
    qrSection: { backgroundColor: WHITE, borderRadius: 20, padding: 20, borderWidth: 1.5, borderColor: "#e2e8f0", alignItems: "center", gap: 12 },
    qrContainer: { padding: 16, backgroundColor: "#f8fafc", borderRadius: 16, alignItems: "center", justifyContent: "center" },
    qrHint: { fontSize: 12, color: "#64748b", fontFamily: "Inter_400Regular", textAlign: "center", marginTop: 4 },
    qrUrl: { fontSize: 10, color: TEAL, fontFamily: "Inter_400Regular", textAlign: "center", marginTop: 2 },
    qrActions: { flexDirection: "row", gap: 8, marginTop: 8, width: "100%", justifyContent: "center", flexWrap: "wrap" },
    actionBtn: { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8 },
    secondaryBtn: { borderWidth: 1.5, borderColor: TEAL, backgroundColor: `${TEAL}08` },
    secondaryBtnText: { fontSize: 12, fontFamily: "Inter_600SemiBold", color: TEAL },
    card: { backgroundColor: WHITE, borderRadius: 20, padding: 20, borderWidth: 1.5, borderColor: "#e2e8f0", gap: 12 },
    sectionTitle: { fontSize: 16, fontFamily: "Inter_700Bold", color: "#1e293b", marginBottom: 8 },
    ...
    infoLabel: { fontSize: 12, fontFamily: "Inter_500Medium", color: "#64748b", textTransform: "uppercase" },
    infoValue: { fontSize: 14, fontFamily: "Inter_600SemiBold", color: "#1e293b" },
    infoValueHighlight: { color: TEAL },
    infoValueDanger: { color: "#ef4444" },
    editFieldGroup: { gap: 6, marginBottom: 12 },
    editLabel: { fontSize: 11, fontFamily: "Inter_600SemiBold", color: "#64748b", textTransform: "uppercase" },
    editInput: { borderWidth: 1, borderColor: "#e2e8f0", borderRadius: 10, paddingHorizontal: 12, paddingVertical: 10, fontSize: 14, fontFamily: "Inter_500Medium", color: "#1e293b", backgroundColor: "#f8fafc" },
    editInputMultiline: { minHeight: 60, textAlignVertical: "top" },
    dangerZone: { marginTop: 16, paddingTop: 20, borderTopWidth: 1, borderTopColor: "#fecaca" },
    dangerBtn: { flexDirection: "row", alignItems: "center", gap: 12, paddingHorizontal: 16, paddingVertical: 12, backgroundColor: "#fef2f2", borderWidth: 1.5, borderColor: "#fecaca", borderRadius: 12 },
    dangerBtnText: { fontSize: 14, fontFamily: "Inter_600SemiBold", color: "#ef4444" },
    infoNote: { flexDirection: "row", backgroundColor: `${TEAL}08`, padding: 12, borderRadius: 12, gap: 10, alignItems: "flex-start" },
    infoNoteText: { flex: 1, fontSize: 12, fontFamily: "Inter_400Regular", color: TEAL_DARK, lineHeight: 16 },
    emptyState: { alignItems: "center", paddingHorizontal: 30, paddingVertical: 40 },
    emptyIcon: { fontSize: 60, marginBottom: 16 },
    emptyTitle: { fontSize: 20, fontFamily: "Inter_700Bold", color: "#1e293b", marginBottom: 8 },
    emptyText: { fontSize: 14, fontFamily: "Inter_400Regular", color: "#64748b", textAlign: "center", marginBottom: 24, lineHeight: 20 },
    createButton: { backgroundColor: TEAL, paddingHorizontal: 24, paddingVertical: 12, borderRadius: 10 },
    createButtonText: { fontSize: 14, fontFamily: "Inter_700Bold", color: WHITE },
    modalOverlay: { flex: 1, backgroundColor: "rgba(0,0,0,0.5)", alignItems: "center", justifyContent: "center" },
    modalContent: { backgroundColor: WHITE, borderRadius: 20, padding: 24, alignItems: "center", gap: 16, marginHorizontal: 20 },
    modalClose: { alignSelf: "flex-end" },
    modalTitle: { fontSize: 18, fontFamily: "Inter_700Bold", color: "#1e293b" },
    modalSubtitle: { fontSize: 13, fontFamily: "Inter_400Regular", color: "#64748b", textAlign: "center", lineHeight: 18 },
});